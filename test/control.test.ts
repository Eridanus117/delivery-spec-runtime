...
const artifacts = ["raw-requirements", "specs", "business-current", "technical-current", "change-plan", "test-plan", "tasks"];
test("严格来源、批准失效、任务状态和投影合同", () => {
  ...
  try {
    ...
    result = runTool("delivery-control.ts", ["guard", "--change-root", change, "--operation", "acceptance"]); assert.equal(result.status, 0, result.stderr);
