...
本文面向需要理解 Runtime 版本绑定、受管投影和失败语义的维护者。接入步骤见[消费仓使用指南](consumer-guide.md)。
...
- OpenSpec 上游生成器不能沿受管路径覆写 Runtime 定制；
- 候选版本不能在真实消费仓试运行；
...
父仓 `HEAD` 中的 gitlink 决定 Runtime commit。受管投影是从该 commit 复制出的普通文件副本，只暴露能力、不承担版本选择；`runtime-check` 逐文件哈希对照 pinned submodule 校验副本，因此不需要 `runtime-lock.json` 或第二份版本状态。
...
4. manifest、Node/OpenSpec 版本和四条受管投影满足合同（逐文件哈希对照 pinned submodule）；
...
| 路径 | 职责 | 是否直接编辑 |
|---|---|---|
| `runtime-manifest.json` | Node 最低版本、OpenSpec 精确版本和四条受管投影合同 | 仅受控 Runtime Change |
...
- **投影漂移：** 只通过 `runtime-link.ts apply` 修复 manifest 托管路径，不手工编辑副本。
