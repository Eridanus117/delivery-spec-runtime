...
- **写给维护者的文字必须是通用中文，以及维护者亲自提出的把关办法**（信号台账 `INT-20260831-014` 的信号11、信号12，均来自 2026-09-01 主会话）。信号11：维护者两次指出 agent 写的汇报与 PR「不说人话」，要求所有进入他视野的文字——口头汇报、PR 标题正文、提交信息、台账、验收记录——都得让不了解本仓内情的人一遍读懂，仓内自造的简称与比喻一律拆成大白话，违反即返工。信号12：维护者当场提出并确认了执行办法——耐久文字（PR 正文、验收记录、README 这类）发出前，先交给一个完全没有上下文的空白会话（例如另开一个 Sonnet 会话）当陌生读者读一遍，圈出看不懂的词句再改写。这个办法的价值在于它不依赖 agent 自我克制，而是引入一个结构上不可能知道内情的第三方来判断，与信号7「要求要活在检查里、不能只活在文字里」同一思路。**重设计时应回答三个问题**：哪些文字必须过这道检查、哪些可免；检查提出问题后是必须改还是可说明理由后照发；这道检查由 agent 自觉发起还是由机器强制。
