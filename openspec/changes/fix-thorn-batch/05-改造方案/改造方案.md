...
| delivery-spec-runtime | `openspec/tools/bootstrap.ts`、`intake-control.ts`、`public-candidate.ts`、`runtime-lib.ts` | 删除未用导入与零调用导出；`runtime-lib.ts` 另新增受管投影校验的纯判据权威定义（见 D-08） | `runtime-lib.ts` 的公开面先收窄后新增两个导出 | 同上 |
| delivery-spec-runtime | `test/*.test.ts` | 新增十组断言（基线 79 项 → 89 项；六组首轮实现，两组为 REV-002 / REV-004 返工补充，两组为收口复审 REV-008 补充）；临时目录清理加有限退避 | 不削弱任何既有断言 | 同上 |
...
- 收口复审 REV-008 的处置过程中查到：`openspec-upgrade.ts`、`render-commands.ts`、`delivery-lifecycle.ts` 三处于基线即存在同形态的入口守卫。维护者裁定本批不修，已登记 `openspec/intake/INT-20260901-024-entry-guard-symlink-fragility.md`。登记时实跑发现其中两处的爆炸半径高于复审最初的评估（`delivery-lifecycle.ts` 在消费仓路径上、`render-commands.ts` 是 CI 步骤），事实与候选修法均记在该条目，同样不构成本 Change 的待决项。
