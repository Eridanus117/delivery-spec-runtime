...
内容摘要（treeDigest）：文件=CRLF→LF 归一化后的字节 sha256（跨仓行尾策略差异不算漂移，Review REV-002 补定）；目录=按名排序后对「相对名+类型+子摘要」求 sha256；投影中出现符号链接即判为旧形态，提示重跑 apply。摘要实时计算、不落盘。「已提交且干净」判定 = 路径被 git 追踪 + `status --porcelain -uall --ignored` 为空（防 .gitignore 隐藏未提交内容被静默覆盖，Review REV-001 补定）。
