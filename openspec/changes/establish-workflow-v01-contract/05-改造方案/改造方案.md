...
本次恢复以“最小可运行闭环”为唯一交付目标：调用方提交一个事项和 Profile，Workflow 返回可继续、需人工判断、阻塞、失败或完成的结构化结果，调用方可以直接使用结果继续下一次执行。运行状态只通过显式请求/结果文件传递，不引入数据库或全局工作台。

本次不实现 Agent 调度器、并发策略、远程指标采集或自动修改规则；先用一个现有 Profile 和合成事项证明人只在判断门介入，后续再建立指标基线。
