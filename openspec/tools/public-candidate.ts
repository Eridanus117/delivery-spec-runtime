...
function generate(runtimeRoot: string, outputRoot: string): void {
  ...
  for (const item of value.paths as string[]) {
  ...
  const report = { schemaVersion: 1, generatedAt: now(), source: "delivery-spec-runtime allowlist", files, checks: { unknownPath: "pass", forbiddenPath: "pass", secretAndEntropy: "pass", examples: "not_in_allowlist" }, externalEffects: "no remote created; no push performed" };
