...
import { validateReport } from "./upgrade-report.ts";
...
// REV-008 通用化：入口模块无条件执行 main()，**绝不加「仅直接执行时才跑」这类守卫**。
// 判据 resolve(process.argv[1]) === resolve(import.meta.filename) 在路径含软链或 junction 时
// 必然为假（ESM 主模块走 realpath，argv[1] 保留调用时的写法），于是 main() 根本不执行，
// 进程以退出码 0、零输出结束——本应被拒的东西被静默放行。
// 这个模块此前留着那道守卫，唯一的理由是它导出 validateReport 供测试 import；
// 该函数已迁往 upgrade-report.ts，本文件从此不导出任何符号，守卫的前提也就消失了。
