...
function guard(root: string, operation: string): void {
  ...
  else if (operation === "release") { guard(root, "acceptance"); if (mode === "delivery") requireAcceptance(root); else { const path = join(root, "08-验收/验收记录.md"); if (!existsSync(path) || !/^结论:\s*(PARTIAL|FAIL|BLOCKED)\s*$/m.test(readFileSync(path, "utf8"))) fail("rehearsal 发布记录前需要非PASS结论"); } }
